...
bool canmov(char src, char dist, int *mov) {
    ...
    if (src == '+' || src == 'S' || src == 'T') return true;
    if (src == '|' && mov[1] == 0) return true;
    if (src == '-' && mov[0] == 0) return true;
    if (src == '.' && mov[0] == 1 && mov[1] == 0) return true;
    return false;
}
...
void dfs(Node src, bool marked[][55], bool re) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!marked[tmp.r][tmp.c]) {
            if ((!re && canmov(map[src.r][src.c], map[tmp.r][tmp.c], mov[i]))
            || (re && canmov(map[tmp.r][tmp.c], map[src.r][src.c], remov[i]))) {
