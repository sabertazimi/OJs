...
search (int r)
{
  ...
  size (r) = 1;
  data (r + n) = data (r) = w[r];
  SysStack[++SysTop].r = r, SysStack[SysTop].pre = 0, SysStack[SysTop].tmp =
    v[r];
  fa (r + n) = 0;
  s1[++top1] = r + n;
  while (SysTop != 0)
	...
	i = SysStack[SysTop].r;
      int
	tmp = SysStack[SysTop].tmp;
      int
	pre = SysStack[SysTop].pre;
      if (tmp == 0)
	{
	  top1--;
	  size (i + n) = size (i);
      ...
      else
	{
	  ...
	  if (hash[j] == 0)
	      ...
	      while (top1 && w[j] < w[s1[top1]])
		{
		  ...
		  elen (s2[top2] + 2 * n) =
		    data (s2[top2]) * ask_road (s2[top2], j);
		}
	      s1[++top1] = j;
	      for (vector < int >::iterator it1 = si[j].begin (), it2 =
		   num[j].begin (); it1 != si[j].end (); it1++, it2++)
		{
		  int
		    pre = *it1;
		  int
