...
describe('LeetCode [0145]', () => {
  test('should AC', () => {
    ...
    expect(solution(arrayToTree([1, 0, 2]))).toStrictEqual([2, 1]);
    expect(solution(arrayToTree([1, 0, 2, 0, 0, 3]))).toStrictEqual([3, 2, 1]);
    expect(solution(arrayToTree([1, 2, 3, 0, 5]))).toStrictEqual([5, 2, 3, 1]);
  });
});
